...
fn assignment_to_non_assignable() {
    ...
    assert!(forward(&mut engine, "3 = 5").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "3 += 5").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "3 -= 5").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "3 *= 5").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "3 /= 5").starts_with("Uncaught Syntax Error: "));
...
fn assign_to_array_decl() {
    ...
    assert!(forward(&mut engine, "[1] = [2]").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "[3, 5] = [7, 8]").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "[6, 8] = [2]").starts_with("Uncaught Syntax Error: "));
    assert!(forward(&mut engine, "[6] = [2, 9]").starts_with("Uncaught Syntax Error: "));
}
...
fn assign_to_object_decl() {
    ...
    const ERR_MSG: &str =
        "Uncaught Parsing Error: expected token \';\', got \':\' in expression statement at line 1, col 3";
